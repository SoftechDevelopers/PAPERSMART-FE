...
const ViewStrength = ({ handleEdit }) => {
  ...
  const fetchStrengths = async () => {
    try {
      ...
      const data = await apiRequest('get', '/strengths', null, {}, signal)

      const strengthsWithSerial = data.map((strength, index) => ({
        ...strength,
        sno: index + 1
      }))
      setStrengths(strengthsWithSerial)
    } catch (error) {
  ...
  // ** Filter strength according to search key
  ...
  const handleDelete = async () => {
    try {
      ...
      const response = await apiRequest('delete', `/strength/${selectedStrength?.strength_id}`, null, {}, signal)
      ...
      if (error.name === 'AbortError') {
        ...
        const message = error?.response?.data?.message ?? 'Error while deleting the strength data'
  ...
  return (
    <Card>
      <CardHeader sx={{ pb: 5 }} title='Strength List' titleTypographyProps={{ variant: 'h6' }} />
      <CardContent>
        ...
        >
          <DialogTitle id='alert-dialog-title'>{'Delete Strength'}</DialogTitle>
          <DialogContent>
